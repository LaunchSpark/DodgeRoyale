...
impl Gym {
    ...
    fn wait(&mut self) -> std::process::ExitStatus {
        self.child.wait().expect("the child exits")
    }
}
...
/// Run a body with a deadline, so a hung child cannot hang the suite.
fn with_deadline<T: Send + 'static>(body: impl FnOnce() -> T + Send + 'static) -> T {
    let (sender, receiver) = mpsc::channel();
    let handle = thread::spawn(move || {
        let _ = sender.send(body());
    });
    receiver.recv_timeout(DEADLINE).map_or_else(
        |elapsed| panic!("the gym did not answer within {DEADLINE:?}: {elapsed}"),
        |value| {
            let _ = handle.join();
            value
        },
